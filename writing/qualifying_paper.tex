...
This `slack' formulation validly specifies that we seek the
weights that make target assignment and it's equivalent cousins the
best solution. Algorithmically, this formulation could lead to many
unnecessary considerations of equivalent constraints, so we might
...
choice is the Hamming loss, $\sum_i^N\begin{cases}
  0 \quad y_i^* = y_i \\
  1 \quad y_i^* \neq y_i
\end{cases}$. It is completely inappropriate for our problem a
