...
\begin{align*}
&\argmin_{\mathbf{w}} \frac{1}{2}||\mathbf{w}||^2 + C\sum\xi_i\\
&\text{such that} \\
&\operatorname{E}(\mathbf{y}, \mathbf{s}, \mathbf{w})
- \operatorname{E}(\mathbf{y}^*, \mathbf{s}, \mathbf{w}) \geq 1 - \xi_i\\ 
&\text{for all } \mathbf{y} \text{ where } \mathbf{y} \text{ is in the set of
  possible neighborhood assignments}\\
&\text{and } \mathbf{y} \neq \mathbf{y}^*
\end{align*}

This 'slack' formulation could validly specify that we seek the weights the
target assignment and it's equivalent cousins the best
solution. 

However, this will be a hard problem to learn. For moderate values of
$C$, the thousands of assignments that have equivalent scores to our
target assignment and therefore $\xi_i$'s of $1$, will completely
dominate the objective, and we will get effectively unregularized
weights. If we set $C$ to be very small, we will also not
appropriately penalize assignments that the target assignment should
be lower than, and we will effectively overregularize the weights.
...
&\text{for all } \mathbf{y} \text{ where } \mathbf{y} \text{ is in the set of
  possible neighborhood assignments,}\\
&\mathbf{y} \not\in \mathcal{Y}^* \text{and } y \text{ is not equivalent to
any other margin violating assignment.}
\end{align*}
%
where $\mathcal{Y}^*$ is the set of assignments that are equivalent to
$y^*$.\footnote{I need to try this}.

Unfortunately, the efficiency of the learning algorithm depends upon
the surprising ease of finding the lowest scoring assignment given a
set of similarity scores (or approximately finding that assignment).
If the lowest scoring assignment happens to be in forbidden set, there
is no known efficient means for finding the next lowest scoring
assignment. This means that there is efficient way to learn this more
correct objective.\footnote{Is this true}



there is no known means to efficiently 


in practice, good performance in out of set prediction
