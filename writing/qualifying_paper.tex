...
\begin{align*}
&\argmin_{\mathbf{w}} \frac{1}{2}||\mathbf{w}||^2 + C\xi\\
&\text{such that} \\
&\operatorname{E}(\mathbf{y}, \mathbf{s}, \mathbf{w})
...
$y^*$.\footnote{I need to try this... The efficiency of
  the learning algorithm depends upon the surprising ease of finding
  the lowest scoring assignment given a set of similarity scores (or
  approximately finding that assignment).  If the lowest scoring
  assignment happens to be in forbidden set, there is no known
  efficient means for finding the next lowest scoring assignment. This
  means that there is efficient way to learn this more correct
  objective. Maybe, I'm not convinced by this argument.}.
