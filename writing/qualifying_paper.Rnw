...
feature_summary <- data.frame(sufficient.population=c(mean(features$all_sufficient), 
                              ...
                              grid.street = c(mean(features$grid_street), 
                                rep(NA, 5)),
                              age.js=c(mean(features$cosine_age[features$all_sufficient==1]), 
                                quantile(features$cosine_age[features$all_sufficient==1])),
                              race.js=c(mean(features$cosine_race[features$all_sufficient==1]), 
                                quantile(features$cosine_race[features$all_sufficient==1])),
                              housing.js=c(mean(features$cosine_housing[features$all_sufficient==1]), 
                                quantile(features$cosine_housing[features$all_sufficient==1])),
                              block.angle=c(mean(features$block_angle[features$all_sufficient==1]), 
                                quantile(features$block_angle[features$all_sufficient==1])),
                              family.js=c(mean(features$cosine_family[features$all_sufficient==1]), 
                                quantile(features$cosine_family[features$all_sufficient==1]))
...
names(weights) <- c('no_neighbor', 'neighbor', colnames(M))
...
combined_pop <- c(as.numeric(pop) 
                  + as.numeric(no_pop), 
                  weights[c('all_sufficient:cosine_family', 
                            'all_sufficient:cosine_ethnicity', 
                            'all_sufficient:cosine_age', 
                            'all_sufficient:cosine_housing')])
