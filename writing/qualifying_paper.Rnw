...
small areal cells. Then for each cell we can attempt to predict the
dominant neighborhood claim. The prediction for a particular cell
would depend upon what we predict for the neighboring cells, the
sociological similarities between the cell and its neighbors, and
whether or not the cells were separated by a physical border. It's a
problem of multinomial regression where all the predictors are in
