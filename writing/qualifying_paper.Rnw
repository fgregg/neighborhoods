...
Within the neighborhoods effect literature, neighborhoods face
collective action problems and much of the variation of social goods
and ills across neighborhoods should be explained by differences in
neighbors' efficacy in responding to these problems.

If we want to understand variation in neighbors' ability to coordinate
a response to common concerns, than we face a fundamental
...
know that for mere proximity is the most important factor for forming
...
The classic example is individual choices of a property owners to
invest in a property. People should invest in their property when they
expect a positive return, but their expectation is related to the
perception that others are investing
\cite{taub_paths_1987}. Similarly, effective informal social control,
like intervening if kids are skipping school, depends upon individual
action influenced by beliefs about how others would
act.\cite{sampson_neighborhoods_1997}

For these perception-based problems, we do not have a good
understanding of what area contains the perceived, relevant co-actors
and the area may well vary for different concerns. However, we can
start with the areas that residents perceive as their neighborhood.
...
boundaries for the same neighborhood.
