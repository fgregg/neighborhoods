...
\begin{figure}
<<<raceImage, dev='pdf'>>=
plot(all_edges$lines[features$sufficient_pop==1], col=rgb(colorRamp(c("lightgrey", "red"))(features$cosine_ethnicity[features$sufficient_pop==1])/255))
@ 
...
\begin{figure}
<<<ageImage, dev='pdf'>>=
plot(all_edges$lines[features$sufficient_pop==1], col=rgb(colorRamp(c("lightgrey", "red"))(features$cosine_age[features$sufficient_pop==1])/255))
@ 
...
\begin{figure}
<<<familyImage, dev='pdf'>>=
plot(all_edges$lines[features$sufficient_pop==1], col=rgb(colorRamp(c("lightgrey", "red"))(features$cosine_family[features$sufficient_pop==1])/255))
@ 
...
plot(all_edges$lines[features$sufficient_pop==1], col=rgb(colorRamp(c("lightgrey", "red"))(features$cosine_housing[features$sufficient_pop==1])/255))
@ 
...
framework\footnote{\url{https://github.com/pystruct/pystruct/tree/rand_loss}},
we get two classes of results: predictions of Chicago neighborhoods
and parameter estimates. 

Let's take up the predictions first:
