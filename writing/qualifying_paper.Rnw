...
of social order \cite{something}. From the place making literature, we
know that local activists and developers also believe that the fate of
an area can depend upon beliefs about what the area is or will be
\cite{something}.
...
with little overlap.\cite{hunter_symbolic_1974,
  campbell_subjective_2009}.  Yet, even with great individual level
variation, there can still be definite districts of a city wherein
residents will tend to perceive that area as their neighborhood.
...
\cite{something}. Many of the listings for sublets, roommates,
...
include a claim that identifiable, geographic location belongs to some 
``Specific location.''
...
appear in a listings's ``Specific location'' field can depends
upon the location of the listing. In other words, the claims for
a particular neighborhood name has a probability distribution over
geographic space over the space of the city. 

For a particular neighborhood, we can imagine that the location of
claims is generated by a draw from this distribution. If so, we can
estimate this probability distribution by looking at the empirical
distribution of observed neighborhood claims.
...
distribution. For the neighborhood case, we will get an estimate of
observing a neighborhood claim at this location. 
...
observing a particular neighborhood claim given a location. This
...
neighborhood from our estimated neighborhood distributions (Figure
\ref{fig:training}). 
...
We will count two blocks as neighbors if they a bordering street or
alley. In the training data, there are \Sexpr{dim(blocks.poly)[1]}
blocks and \Sexpr{dim(features)[1]} block-neighbors.
