...
world into meaningful components. In recent years we have
developed many methods to accomplish this when know what parts of the
world are similar to one another--clustering, community
...
atomic, spatial unit for urban processes.\citep{palmer_primary_1932}
...
taking the square root, $\chi$ becomes a metric.\citep{pele_distance_2011}
...
framework,\citep{mueller_pystruct:_????} we get two classes of results:
