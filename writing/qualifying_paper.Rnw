...
guest house, etc. We also use the absolute difference between the log
median density of housing units.

Many blocks have no one living in them or nearly so. In such
...
close to 0. The relatively strong `attractive' parameter for
differences in family structure is surprising. According this
model, a block that is all families and a block that has no families
are apt to belong the same neighborhood. 
...
is because people, like social scientists, have a difficult time
