...
opts_chunk$set(echo=FALSE, 
               ...
               prefix.cache='/home/fgregg/sweave-cache/values', 
               dev='tikz',
               cache=TRUE) 
...
\begin{figure}
<<zoningImage, dev='pdf'>>= 
plot(all_edges$lines, col=c("#00000022", "red")[features$zoning + 1])
@ 
...
\section*{Results}
<<trainModel, include=FALSE, eval=FALSE>>= 
setwd('../code/training/')
system('python train_ssvm.py 0.001')
...
WT <- xtable(t(weights), digits=4)
row.names(WT) <- dimnames(M)[[2]]
print(WT)
@ 

<<processHoodLabels, echo=FALSE>>=
setwd('../code/training/')
...
@ 
\begin{figure}
<<plotPredictions, cache=FALSE, dev='pdf'>>=
plot(blocks.poly, col=rgb(t(col2rgb(sample(colors())[hoods %% 657])/255), alpha=ifelse(hood_frequency[hoods] < 10, 0.05, 1)), border="transparent")
@ 
