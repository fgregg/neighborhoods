...
As scientists and people, we cannot fail to notice the patterns of
human action and interaction. Unfortunately, we can easily fail to
capture and measure those patterns, let alone account for them.
Patterns and concepts that first appeared so clear become hazy
when we attempt to define them precisely. The `neighborhood' is a
classic case.

Researchers, like city dwellers, see that cities contain areas where
the patterns of action and interaction are markedly different than
neighboring zones. From that seed of perception, scholars have 
grown a thicket of definitions about the neighborhood and theories
about it--what it means, how it produces its distinctive patterns, how
...
them. Our next move should not be to theorize about what kinds of
