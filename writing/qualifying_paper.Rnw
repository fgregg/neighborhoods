...
As a name, `neighborhood effect' has produced some confusion, as it
has directed attention to the neighborhood as an area and away from
groups of residents facing common coordination problems. When we focus
on the residents and their problems, we see the variety of types of
problems, coordination mechanisms, and characteristic
geographies. Each problem has its own neighborhood.
...
neighbor depends very strongly on distance. Most people know their
...
very little control of the values of property in their area. Except in
...
the future of the area. However, unlike informal social control or
...
beliefs and perceptions evolve about areas.

We know very little about a.) how residents come to perceive some
...
to study how a thing if we can't ever locate the thing.

Some researchers have attempted to identify perceived neighborhoods by
surveying city residents, typically by asking them to draw maps
of their neighborhood. These studies persistently find that residents
draw very idiosyncratic maps, often with little
overlap\cite{hunter_symbolic_1974, campbell_subjective_2009}.
However, even with great individual level variation, there can still be
definite districts of a city which residents will tend to perceive as
their neighborhood. We need much more data to uncover them.
...
neighborhood, these methods provide the grounds for investigation for
how the beliefs and reputations of places affect the fact of an area,
and how those beliefs, perceptions, and reputations can be transformed.
...
\cite{_craigslist_????}. Craigslists has many categories of listings
including listings for apartments, sublets, roommates. Many of these housing
listings contain claims that the advertised location is part of a
particular neighborhood. In aggregate, these claims can inform us
about neighborhood perception.
...
individuals (about 66\% and 85\% respectively). 
...
of a neighborhood. We'll be using this claim to estimate the location
...
neighborhood can have very different ideas about the contours of the
neighborhood\cite{hunter_symbolic_1974,
  campbell_subjective_2009}. With the volume of claims we have from
Craigslist, we will be able to average out this idiosyncrasy and find
the average perceived neighborhood. We will locate the parts of the
city that residents tend to claim as the same neighborhood.

We'll do this in two steps. First, for each neighborhood, we'll
estimate a probability distribution of seeing that claim anywhere in
the city. Second, we'll compare each neighborhood's estimated
distribution to find areas where a particular neighborhood name
predominates.

Of course, advertiser's claims culled from an Internet source are an
imperfect measure of resident's perception. The claims are biased
by both selection and strategy. 

As we are using listings for apartments, sublets, and roommates, we
have much less data from homeowners. Moreover, listers are
...
The probability of the location of a neighborhood claim depends upon
the neighborhood. For a particular neighborhood name, we can imagine
that the location of a particular claim is generated by a random draw
from that names geographic probability distribution. We can estimate
...
fall very sharply and unpredictably across the space of the city, we
...
densities, we get an overall probability density of observing a
neighborhood claim at this location.
...
every neighborhood name we observe. With this in hand, we have an
estimate of the probability of seeing a neighborhood claim at any
location in the city for any neighborhood:
$\hat{\Pr}(\text{Location}_j \mid \text{Hood}_i)$

Next, we transform the probabilities of locations given a neighborhoods
into the probabilities of observing a particular neighborhood claim
given a location, $\hat{\Pr}(\text{Hood}_i \mid
\text{Location}_j)$. This depends not just on the probability of
seeing a claim for a particular neighborhood, but will also depend
upon probabilities for of seeing a claim all neighborhoods.
...
neighborhood claiming was very idiosyncratic, then we would expect
that that would be neighborhood names that dominated two or more
separate, disconnected areas. Instead we see definite districts of the
city wherein Craigslist advertisers tend to claim that area is a
neighborhood. 
...
we have already discussed, we have reason to fear that claims are a
biased measure. To build confidence, we show that these neighborhood
areas line up with the physical barriers and sociological similarities
and differences that urban sociologists have long associated with
neighborhood boundaries. We'll make the case first visually and
impressionalistically and then formally and mathematically.

In the Figure \ref{fig:prob}, the map shows the large parks, the
...
discrete variables requires that we calculate terms for every possible
configuration of assignments between neighborhood claims and blocks. With
...
finding the minimum cut of a network in polynomial time. 
